use crate::types::{AssetClass, Currency, Money, Percentage};
...
impl Portfolio {
    ...
    pub fn total_value(&self) -> Money {
        ...
        let total = self.assets.iter().map(|a| a.current_value.amount()).sum();
    ...
    pub fn total_cost_basis(&self) -> Money {
        ...
        let total = self.assets.iter().map(|a| a.cost_basis.amount()).sum();
    ...
    pub fn get_allocations(&self) -> Vec<AssetAllocation> {
        ...
        for asset in &self.assets {
            let entry = allocations.entry(asset.asset_class.clone()).or_insert((
                Money::new_unchecked(Decimal::ZERO, asset.current_value.currency()),
                None,
            ));
        ...
        allocations
            ...
            .map(|(asset_class, (value, target))| {
                ...
                let deviation = target.map(|t| {
                    Percentage::from_percentage(current_weight.as_percentage() - t.as_percentage())
                        .unwrap()
                });
...
mod tests {
    ...
    fn test_asset_gain_loss() {
        ...
        assert_eq!(
            asset.unrealized_gain_loss_percentage().as_percentage(),
            dec!(25)
        );
    }
}
