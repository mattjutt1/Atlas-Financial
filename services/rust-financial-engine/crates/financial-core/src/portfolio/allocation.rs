use crate::portfolio::types::{Asset, RiskTolerance, TradeAction, TradeRecommendation};
use crate::types::{AssetClass, Percentage};
use crate::{FinancialError, Money, Portfolio, Result};
...
impl AllocationStrategist {
        ...
        vec![
            ...
            StrategicAllocation {
                ...
                target_allocations: [
                    ...
                    (
                        AssetClass::Bonds,
                        Percentage::from_percentage(dec!(70.0)).unwrap(),
                    ),
                    (
                        AssetClass::Stocks,
            ...
            StrategicAllocation {
                ...
                target_allocations: [
                    (
                        AssetClass::Cash,
                        Percentage::from_percentage(dec!(5.0)).unwrap(),
                    ),
                    (
                        AssetClass::Bonds,
                        Percentage::from_percentage(dec!(40.0)).unwrap(),
                    ),
                    (
                        AssetClass::Stocks,
                        Percentage::from_percentage(dec!(50.0)).unwrap(),
                    ),
                    (
            ...
            StrategicAllocation {
                ...
                target_allocations: [
                    (
                        AssetClass::Stocks,
                        Percentage::from_percentage(dec!(80.0)).unwrap(),
                    ),
                    (
                        AssetClass::RealEstate,
                        Percentage::from_percentage(dec!(10.0)).unwrap(),
            ...
            StrategicAllocation {
                ...
                target_allocations: [
                    (
                        AssetClass::Stocks,
                        Percentage::from_percentage(dec!(30.0)).unwrap(),
                    ),
                    (
                        AssetClass::Bonds,
                        Percentage::from_percentage(dec!(55.0)).unwrap(),
                    ), // 40% long-term + 15% intermediate
                    (
                        AssetClass::Commodities,
                        Percentage::from_percentage(dec!(7.5)).unwrap(),
                    ),
                    (
                        AssetClass::RealEstate,
                        Percentage::from_percentage(dec!(7.5)).unwrap(),
                ...
                description:
                    "Risk parity approach designed to perform well in all economic environments"
                        .to_string(),
            },
    ...
    pub fn analyze_allocation(
        ...
        let target_allocations =
            self.calculate_target_allocations(portfolio, &target_allocation.target_allocations)?;
        ...
        for asset_class in target_allocation.target_allocations.keys() {
            let current_pct = current_allocations
                .get(asset_class)
    ...
    pub fn age_based_allocation(
        &self,
        age: u32,
        risk_tolerance: RiskTolerance,
    ) -> Result<StrategicAllocation> {
        ...
        target_allocations.insert(
            AssetClass::Stocks,
            Percentage::from_percentage(Decimal::from(stock_pct))?,
        );
        target_allocations.insert(
            AssetClass::Bonds,
            Percentage::from_percentage(Decimal::from(bond_pct))?,
        );
        target_allocations.insert(
            AssetClass::RealEstate,
            Percentage::from_percentage(dec!(5.0))?,
        );
    ...
    pub fn lifecycle_glide_path(
        &self,
        current_age: u32,
        retirement_age: u32,
    ) -> Result<Vec<(u32, StrategicAllocation)>> {
    ...
    fn calculate_current_allocations(
        ...
        for asset in &portfolio.assets {
            let percentage =
                Percentage::from_decimal(asset.current_value.amount() / total_value.amount())?;
    ...
    fn generate_rebalancing_trades(
        ...
        for (asset_class, target_pct) in target_allocations {
            ...
            if let Some(assets) = assets_by_class.get(asset_class) {
                let current_value: Decimal = assets.iter().map(|a| a.current_value.amount()).sum();
                ...
                if value_diff.abs() > total_value.amount() * self.rebalancing_threshold.as_decimal()
                {
                    ...
                    if let Some(largest_asset) =
                        assets.iter().max_by_key(|a| a.current_value.amount())
                    {
                        ...
                        trades.push(TradeRecommendation {
                            ...
                            action,
                            quantity,
                            estimated_value: Money::new_unchecked(
                                trade_value,
                                total_value.currency(),
                            ),
                            current_weight: Percentage::from_decimal(
    ...
    fn calculate_transaction_costs(&self, trades: &[TradeRecommendation]) -> Money {
        let total_trade_value: Decimal = trades.iter().map(|t| t.estimated_value.amount()).sum();
...
mod tests {
    ...
    fn test_strategic_models() {
        ...
        let conservative = models
            .iter()
            .find(|m| m.model_name == "Conservative Income")
            .unwrap();
        assert_eq!(conservative.risk_tolerance, RiskTolerance::Conservative);

        let all_weather = models
            .iter()
            .find(|m| m.model_name == "All Weather")
            .unwrap();
    ...
    fn test_age_based_allocation() {
        ...
        let young_allocation = strategist
            .age_based_allocation(25, RiskTolerance::Aggressive)
            .unwrap();
        let stock_allocation = young_allocation
            .target_allocations
            .get(&AssetClass::Stocks)
            .unwrap();
        assert!(stock_allocation.as_percentage() > dec!(70)); // Should be aggressive

        let older_allocation = strategist
            .age_based_allocation(60, RiskTolerance::Conservative)
            .unwrap();
        let stock_allocation_older = older_allocation
            .target_allocations
            .get(&AssetClass::Stocks)
            .unwrap();
    ...
    fn test_lifecycle_glide_path() {
        ...
        let first_stocks = first_allocation
            .target_allocations
            .get(&AssetClass::Stocks)
            .unwrap();
        let last_stocks = last_allocation
            .target_allocations
            .get(&AssetClass::Stocks)
            .unwrap();
