...
export class AtlasError extends Error {
    constructor(message, code, category, statusCode = 500, isRetryable = false, suggestions = [], metadata = {}) {
        super(message);
...
export class AuthenticationError extends AtlasError {
    constructor(message, metadata = {}) {
        super(message, 'AUTH_FAILED', 'authentication', 401, false, [
...
export class AuthorizationError extends AtlasError {
    constructor(message, resource, metadata = {}) {
        super(message, 'AUTHZ_FAILED', 'authorization', 403, false, [
...
export class TokenExpiredError extends AtlasError {
    constructor(metadata = {}) {
        super('Authentication token has expired', 'TOKEN_EXPIRED', 'authentication', 401, false, [
...
export class InvalidTokenError extends AtlasError {
    constructor(reason, metadata = {}) {
        super(`Invalid JWT token: ${reason}`, 'INVALID_TOKEN', 'authentication', 401, false, [
...
export class ValidationError extends AtlasError {
    constructor(field, message, metadata = {}) {
        super(`Validation error: ${field} - ${message}`, 'VALIDATION_ERROR', 'validation', 400, false, [
...
export class InvalidInputError extends AtlasError {
    constructor(message, metadata = {}) {
        super(`Invalid input format: ${message}`, 'INVALID_INPUT', 'validation', 400, false, [
...
export class MissingFieldError extends AtlasError {
    constructor(field, metadata = {}) {
        super(`Missing required field: ${field}`, 'MISSING_FIELD', 'validation', 400, false, [
...
export class NotFoundError extends AtlasError {
    constructor(resourceType, resourceId, metadata = {}) {
        super(`${resourceType} not found: ${resourceId}`, `${resourceType.toUpperCase()}_NOT_FOUND`, 'not_found', 404, false, [
...
export class ConflictError extends AtlasError {
    constructor(message, metadata = {}) {
        super(message, 'CONFLICT', 'conflict', 409, false, [
...
export class ExternalServiceError extends AtlasError {
    constructor(service, message, metadata = {}) {
        super(`${service} error: ${message}`, `${service.toUpperCase()}_ERROR`, 'external', 502, true, [
            'Retry the operation',
...
export class DatabaseError extends AtlasError {
    constructor(message, metadata = {}) {
        super(`Database error: ${message}`, 'DATABASE_ERROR', 'external', 503, true, [
            'Retry the operation',
...
export class CacheError extends AtlasError {
    constructor(message, metadata = {}) {
        super(`Cache error: ${message}`, 'CACHE_ERROR', 'external', 503, true, [
...
export class RateLimitError extends AtlasError {
    constructor(limit, window, retryAfter, metadata = {}) {
        super(`Rate limit exceeded: ${limit} requests per ${window}`, 'RATE_LIMIT_EXCEEDED', 'throttling', 429, true, [
...
export class TimeoutError extends AtlasError {
    constructor(timeoutMs, metadata = {}) {
        super(`Request timeout after ${timeoutMs}ms`, 'REQUEST_TIMEOUT', 'throttling', 408, true, [
            'Retry the request',
...
export class InternalError extends AtlasError {
    constructor(message, metadata = {}) {
        super(`Internal server error: ${message}`, 'INTERNAL_ERROR', 'system', 500, false, [
...
export class ConfigurationError extends AtlasError {
    constructor(message, metadata = {}) {
        super(`Configuration error: ${message}`, 'CONFIG_ERROR', 'system', 500, false, [
...
export class ServiceUnavailableError extends AtlasError {
    constructor(service, metadata = {}) {
        super(`Service unavailable: ${service}`, 'SERVICE_UNAVAILABLE', 'system', 503, true, [
            'Retry the request',
            'Check service status',
...
export class FinancialCalculationError extends AtlasError {
    constructor(calculation, message, metadata = {}) {
        super(`Financial calculation error in ${calculation}: ${message}`, 'FINANCIAL_CALCULATION_ERROR', 'financial', 400, false, [
...
export class CurrencyMismatchError extends AtlasError {
    constructor(expectedCurrency, actualCurrency, metadata = {}) {
        super(`Currency mismatch: expected ${expectedCurrency}, got ${actualCurrency}`, 'CURRENCY_MISMATCH', 'financial', 400, false, [
 ...
 */
export function createValidationErrors(validationResults) {
    return validationResults.map(result => new ValidationError(result.field, result.message));
}
/**
 ...
 */
export function getRetryDelay(error, attemptCount) {
...
export const ErrorFactory = {
    validation: (field, message, metadata) => new ValidationError(field, message, metadata),
    notFound: (resourceType, resourceId, metadata) => new NotFoundError(resourceType, resourceId, metadata),
    unauthorized: (message = 'Authentication required', metadata) => new AuthenticationError(message, metadata),
    forbidden: (message = 'Access denied', resource, metadata) => new AuthorizationError(message, resource, metadata),
    conflict: (message, metadata) => new ConflictError(message, metadata),
    external: (service, message, metadata) => new ExternalServiceError(service, message, metadata),
    internal: (message, metadata) => new InternalError(message, metadata),
    rateLimit: (limit, window, retryAfter, metadata) => new RateLimitError(limit, window, retryAfter, metadata),
    timeout: (timeoutMs, metadata) => new TimeoutError(timeoutMs, metadata)
};
/**
 * Error type guards
 */
export const isAtlasError = (error) => error instanceof AtlasError;
...
export const isRetryableError = (error) => error instanceof AtlasError && error.isRetryable;
