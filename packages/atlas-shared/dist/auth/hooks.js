...
export function useRole(requiredRole) {
    ...
    return {
        hasRole: user?.roles.includes(requiredRole) || false,
        roles: user?.roles || [],
        user
    };
}
...
export function usePermission(resource, action) {
    const { user } = useAuthentication();
    const hasPermission = user?.permissions.some(permission => permission.resource === resource &&
        permission.action === action) || false;
    return {
        hasPermission,
...
export function useUserProfile() {
    ...
    const getProfileCompleteness = () => {
        ...
        const completedFields = fields.filter(field => user[field] !== undefined &&
            user[field] !== '').length;
