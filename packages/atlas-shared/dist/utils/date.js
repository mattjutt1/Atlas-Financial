 ...
 */
export const formatDate = (date, pattern = 'MMM dd, yyyy', options) => {
    try {
 ...
 */
export const formatDateTime = (date, pattern = 'MMM dd, yyyy h:mm a') => {
 ...
 */
export const formatRelativeTimeCustom = (date) => {
    try {
        const dateObj = typeof date === 'string' ? parseISO(date) : date;
        const now = new Date();
        ...
        if (diffInSeconds < 0) {
            ...
            if (futureDiff < 3600)
                return `in ${Math.floor(futureDiff / 60)}m`;
            if (futureDiff < 86400)
                return `in ${Math.floor(futureDiff / 3600)}h`;
            if (futureDiff < 2592000)
...
export const formatInTimezone = (date, timezone, pattern = 'yyyy-MM-dd HH:mm:ss') => {
    try {
        const dateObj = typeof date === 'string' ? parseISO(date) : date;
